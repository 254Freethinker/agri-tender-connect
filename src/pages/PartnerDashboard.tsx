...
import { 
  ...
  PartnerProfileValues,
  partnerSearchFiltersSchema
...
// Components
import PartnerEventForm from '@/components/partnerships/PartnerEventForm';
import PartnerEventsList from '@/components/partnerships/PartnerEventsList';
...
const PartnerDashboard: React.FC = () => {
  ...
  const handleEventSubmit = async (eventData: Omit<PartnerEvent, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      let result;
      if (editingEvent) {
        result = await updatePartnerEvent(editingEvent.id, eventData);
        toast({
          title: 'Success',
          description: 'Event updated successfully!',
        });
      } else {
        result = await createPartnerEvent(eventData);
        toast({
          title: 'Success',
  ...
  const handleDeleteEvent = async (eventId: string) => {
    ...
    try {
      const { error } = await deletePartnerEvent(eventId);
